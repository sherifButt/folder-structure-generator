const {
   ...
   parsePayload,
   createOutputDir,
} = require('../helpers/structureFileHelpers')
...
exports.uploadedFileHandler = async (req, res, next) => {
   if (!req.file) {
      return res
         ...
         .json({
            message:
               'No file was uploaded. Please provide a valid structure file.',
         })
   }
...
exports.payloadHandler = async (req, res, next) => {
   try {
      const { folderStructure, messages, meta } = parsePayload(
         req.payload,
         req.file.mimetype
      )
...
exports.directoryHandler = async (req, res, next) => {
   ...
   try {
      ...
      return res
         .status(500)
         .json({
            message: 'Error creating directory structure: ' + error.message,
         })
   }
}
...
exports.zipHandler = async (req, res, next) => {
   try {
      ...
      return res
         .status(500)
         .json({ message: 'Failed to zip the folder: ' + error.message })
   }
}
